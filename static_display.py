...
def DP_run(control_type = "None", animation = True, static_plot = True, speed = 1):
    ...
    if static_plot:
        ...
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(10, 9), sharex=True, constrained_layout=True)
        fig.suptitle(f'Double pendulum - control: {control_type} (cost = {total_cost:.2f})')
        timestamp_figure(fig)

        ax1.plot(t_array, solution[:, 0], label='Cart Position (m)')
        ax1.set_ylabel('Position (m)')

        ax2.plot(t_array, solution[:, 1], label='Pendulum Angle 1 (rad)', color='orange')
        ax2.axhline(np.pi, color='k', lw=0.5, ls='--')
        ax2.set_ylabel('Angle 1 (rad)')

        ax3.plot(t_array, solution[:, 2], label='Pendulum Angle 2 (rad)', color='red')
        ax3.axhline(np.pi, color='k', lw=0.5, ls='--')
        ax3.set_ylabel('Angle 2 (rad)')

        ax4.plot(t_array, solution[:, 6], label='Motor Force (N)', color='green')
...
if __name__ == "__main__":
    ...
    # SP_run(control_type = "ML", animation = True, static_plot = True, speed = 0, network = 0, y0 = [100, np.pi, 0, 0])
    # SP_run(control_type = "ML", animation = True, static_plot = True, speed = 0, network = 0, y0 = [-100, np.pi, 0, 0])
    SP_run(control_type = "ML", animation = True, static_plot = True, speed = 0, network = 0, y0 = [0, np.pi, 0, 0])
    # DP_run(control_type = 'inverted_rod_2', animation = True, static_plot = True)
