...
log_dir = 'training_logs'  # Per-run .npz files of the diagnostics, for later review
log_interval = 25          # Episodes between log writes, so a stopped run keeps its data
...
class RL_trainer:
    ...
    def __init__(self, model):
        ...
        self.history = None       # the live diagnostic lists, once train() is running
        self.run_started = None   # names the log file, so one run writes one file
        self.log_floor = -3
        self.log_ceiling = 3
        self.d_log_std = 0
        ...
        self.NN_V.theta_generate(bias_loc = [0, 0, 32], seed = 1)
        self.NN_mu.theta_generate(seed = 1)
    ...
    def save_training_log(self):
        """Write this run's diagnostics to training_logs/run_<start time>.npz.

        Called every `log_interval` episodes and again at the end, always to the
        same file for a given run, so stopping a run mid-flight still leaves a
        log that plot_training_log() can open.
        """
        if not self.history or not self.history['reward']:
            return None

        os.makedirs(log_dir, exist_ok = True)
        path = os.path.join(log_dir,
                            f'run_{self.run_started.strftime("%Y-%m-%d_%H-%M-%S")}.npz')
        np.savez(path,
                 started = self.run_started.strftime('%Y-%m-%d %H:%M:%S'),
                 **{name: np.asarray(values) for name, values in self.history.items()})
        return path
    ...
    def train(self, variance = 0, max_runtime = 60):
        ...
        # The same list objects the loop appends to, so a log written at any point
        # (including from an interrupt) sees everything recorded so far.
        self.history = {'reward': reward_history,
                        'log_std': log_std_history,
                        'advantage': advantage_history,
                        'signed_advantage': signed_advantage_history,
                        'full_sides': full_sides_history,
        ...
        print('training log saved to', self.save_training_log())
        plt.ioff()
        plt.show()


def latest_training_log():
    """Path of the most recently written run log, or None if there are none."""
    logs = glob.glob(os.path.join(log_dir, 'run_*.npz'))
    return max(logs, key = os.path.getmtime) if logs else None


def plot_training_log(path = None):
    """Draw a saved run as a static figure - the live diagnostics, after the fact.

    `path` defaults to the newest log in training_logs/. Returns the figure so a
    caller can save it; run as `python RL_training.py plot [path]` to just look.
    """
    path = path or latest_training_log()
    if path is None:
        print(f'no run logs in {log_dir}/ - train first, or pass a path')
        return None

    log = np.load(path, allow_pickle = True)
    reward = log['reward']
    episodes = np.arange(len(reward))
    full_sides = log['full_sides']
    best_episodes, best_rewards = log['best_episodes'], log['best_rewards']

    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize = (12, 8), sharex = True,
                                        constrained_layout = True)
    fig.canvas.manager.set_window_title(os.path.basename(path))
    fig.suptitle(f'{os.path.basename(path)} - {len(reward)} episodes, '
                 f'run started {log["started"]}', fontsize = 10)
    timestamp_figure(fig)

    # Same reading as the live plot: dots colored by how many of the two starting
    # sides lasted the full runtime, stars on new bests, green for the evaluations.
    ax1.plot(episodes, reward, color = '0.75', lw = 1, zorder = 1)
    for sides_done, color, label in ((2, 'tab:blue', 'both sides full'),
                                     (1, 'gold', 'one side full'),
                                     (0, 'tab:red', 'neither side full')):
        keep = full_sides == sides_done
        ax1.plot(episodes[keep], reward[keep], linestyle = 'none', marker = 'o',
                 markersize = 2, color = color, label = label, zorder = 2)
    if len(best_episodes):
        # Step-style running best: hold each record until the next one replaces it
        ax1.plot(list(best_episodes) + [len(reward) - 1],
                 list(best_rewards) + [best_rewards[-1]],
                 linestyle = '--', drawstyle = 'steps-post', color = '0.6', lw = 1, zorder = 2)
        ax1.plot(best_episodes, best_rewards, linestyle = 'none', marker = '*',
                 markersize = 7, color = 'tab:orange', label = 'new max', zorder = 5)
    if len(log['eval_episodes']):
        ax1.plot(log['eval_episodes'], log['eval_rewards'], marker = 'o', markersize = 3,
                 color = 'tab:green', lw = 1, label = 'eval (no noise)', zorder = 4)
    ax1.set_ylabel('Reward')
    ax1.legend(loc = 'lower right', fontsize = 'small', ncol = 2)

    ax2.plot(episodes, log['log_std'])
    ax2.set_ylabel('Log Std')

    ax3.plot(episodes, log['advantage'], label = '|advantage|')
    ax3.plot(episodes, log['signed_advantage'], label = 'signed')
    ax3.set_ylabel('Mean Advantage')
    ax3.set_xlabel('Episode')
    ax3.legend(loc = 'upper right', fontsize = 'small')
...
if __name__ == "__main__":
    # `python RL_training.py plot [path]` reviews a finished run instead of training.
    if len(sys.argv) > 1 and sys.argv[1] == 'plot':
        plot_training_log(sys.argv[2] if len(sys.argv) > 2 else None)
    else:
        y0 = [0, np.pi, 0, 0]
        ...
        try:
            ...
            # Ctrl-C is the normal way to end a run, so keep the log rather than
            # losing the episodes since the last periodic write.
            print('\ninterrupted - training log saved to', main.save_training_log())
            print('review it with:  python RL_training.py plot')