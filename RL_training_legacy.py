...
class RL_trainer:
    ...
    def reward(self, state):
        ...
        base_reward =  (25 - angle_cf * math.cos(state[1]) - location_cf * (state[0])**2 + (0.02/(state[0]**2+0.01))) / 10
    ...
    def normalize(self, state):
        ...
        return np.array([-1/(abs(state[0])+1)+1, np.sin(state[1]), state[2], np.cos(state[3])])
    ...
    def train(self, variance = 0):
        ...
        for episode in range(10000):
            ...
            random_angle = np.pi
            random_location = np.random.choice([-1, 1])
            ...
            for side in sides:
                self.model.state = [side * random_location, side * random_angle, 0, 0]
                t = 0
                done = False
                while not done:
                    t += 1
                    ...
                    done = next_state[1] <= np.pi/2 or next_state[1] >= 3*np.pi/2 or t >= (self.model.t_end - self.model.t_start) * self.model.fps or abs(next_state[2]) > 100 or abs(next_state[3]) > 100 or abs(next_state[0]) > 3.0
...
y0 = [0, np.pi, 0, 0]
SP = redone_legacy.SinglePendulum(params=(9.8, 1, 1, 1), y0 = y0,t_end=60)
main = RL_trainer(SP)
